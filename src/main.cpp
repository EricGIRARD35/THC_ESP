...
// Entrées passthrough CNC (recopie du step/dir venant de la carte CNC)
...
#define ADS1115_ADDR_GND 0x48   // Pin ADDR connectée à GND -> adresse I2C 0x48
#define ADS_PLASMA_CHANNEL 1    // Canal de l'ADS1115 utilisé pour lire le voltage plasma (AIN0)
...
// Pleine échelle de l'ADS1115 (FSR) en volts.
// Gain configuré à GAIN_ONE => ±4.096V. Assurez-vous que la tension après le
// diviseur reste toujours inférieure à cette valeur.
const float ADS_GAIN_V = 4.096;

// Résolution de l'ADS1115 (2^15 pour une lecture single-ended signée)
const float ADS_RESOLUTION_MAX = 32767.0;

// Rapport du diviseur de tension (Exemple : si 40:1, alors K = 1/40 = 0.025)
// C'est le coefficient K tel que V_plasma = V_mesurée / K
float PLASMA_VOLTAGE_DIVIDER_RATIO = 76.4; // Vérifiez votre diviseur réel !

// Lecture tactile non-bloquante
...
const float DEFAULT_SETPOINT = 100.0; // Attention : stocké sur 4 octets en EEPROM mais utilisé en double pour le PID
static float slow_lp = 0.0f;
const float DEFAULT_STEP_PER_MM = 400;
const float DEFAULT_KP = 1;   // L'action proportionnelle agit ici comme une action intégrale : elle pilote la vitesse du moteur Z, pas sa position.
const float DEFAULT_KI = 0.5;
const float DEFAULT_KD = 0;   // Action dérivée laissée à 0
...
#define MM_PER_VOLT_Z 1.0 // À ajuster lors des essais
...
PID myPID(&Input, &Output, &Setpoint, Kp, Ki, Kd, REVERSE); // REVERSE : l'action du PID doit être inverse de l'erreur
long z_target = 0;
long last_z_target = 0;
const float PID_TO_STEPS = 10.0; // mm/V à ajuster
...
int prevCLK; // État précédent de CLK
int prevDT;  // État précédent de DT

// Mesure du temps d'exécution de la boucle principale
...
// Méthode anti-dive
float fast_voltage = 0.0;              // Tension filtrée rapide (corrigée)
float slow_voltage = 0.0;              // Tension filtrée lente (corrigée)
bool anti_dive_active = false;         // État anti-dive
unsigned long anti_dive_start_time = 0;// Horodatage de déclenchement anti-dive
bool enable_was_active = false;
volatile bool enable_active_g = false;

// Historique de position (pour amélioration future de l'anti-dive)
const int POSITION_HISTORY_INTERVAL = 100; // ms entre chaque enregistrement
const int POSITION_HISTORY_SIZE = 20;      // ~2 secondes d'historique
struct PosHistory {
  unsigned long time;
  long position;
};
...
#define OVERSAMPLE_TARGET 6  // échantillons avant calcul de la moyenne
float oversample_sum = 0.0;
uint8_t oversample_count = 0;
float last_pid_input = 0.0;    // Dernière moyenne pour le low-pass
const float INPUT_ALPHA = 0.7; // Low-pass fort sur la moyenne
...
// === ANTI-DIVE : actif uniquement si le THC est actif ===
...
// ========================================
// SIMULATEUR DE SINUSOÏDE (pour test du PID)
// ========================================

bool simulation_mode = false;
unsigned long simulation_start_time = 0;
float simulation_amplitude = 3.0;      // Amplitude autour du setpoint
float simulation_frequency = 0.1;      // Fréquence en Hz
float simulation_offset = 2.0;         // Offset DC pour tester le tracking
...
void IRAM_ATTR handleCNCStep() {
  // On ne laisse passer les pas de la CNC que si le THC ne pilote pas l'axe
  if (!enable_active_g) {
...
void taskLvglTick(void *pvParameters) {
    for (;;) {
        lv_tick_inc(1);                 // Tick LVGL à 1 ms
        vTaskDelay(pdMS_TO_TICKS(1));   // Cadencé à 1 kHz
    }
}
...
void taskUI(void *pvParameters) {
    ...
    // Copie locale des données partagées
    ...
    for (;;) {
        ...
        // Copie des données avec protection mutex
        if (xSemaphoreTake(dataMutex, pdMS_TO_TICKS(5)) == pdTRUE) {
            local_data = display_data;
            xSemaphoreGive(dataMutex);
        }

        // Mise à jour des labels (avec les données locales)
...
void setup() {
  ...
  // --- 1. Initialisation EEPROM (émulée en flash sur ESP32) ---
  Serial.begin(115200);
  delay(1000);
  ...
  // Initialise l'EEPROM avec les valeurs par défaut si elle n'est pas déjà initialisée
  initializeEEPROM();
  ...
  // --- 3. I2C ---
  Wire.begin(ADS_I2C_SDA_PIN, ADS_I2C_SCL_PIN); // SDA(17) / SCL(22)
  ...
  tft.init();
  tft.setRotation(1);     // Rotation paysage pour ce module ESP32/TFT
  tft.fillScreen(TFT_BLACK);

  // Init du tactile XPT2046 après le TFT
  if (!ts.begin()) {
      Serial.println("XPT2046 échec!");
  }
  ts.setRotation(1);

  // Initialisation LVGL (après le TFT)
  lvgl_setup();
  ...
  // Validation des valeurs chargées depuis l'EEPROM
  ...
  // --- 4. Résolution ADC ---
  // L'ESP32 est limité à 12 bits (0-4095) ; diviser par 4 les calculs prévus pour 14 bits (16383).
  analogReadResolution(12);

  // Initialisation de l'historique de position
...
void simulateSineWave() {
    ...
    // Input = Setpoint + A*sin(2πft) + offset
    float angle = 2.0 * PI * simulation_frequency * time_sec;
    Input = Setpoint + simulation_amplitude * sin(angle) + simulation_offset;

    // Marque input_ready pour que le PID calcule
    input_ready = true;

    // Simule aussi fast_voltage/slow_voltage pour l'affichage
    fast_voltage = Input;
    slow_voltage = Input;
...
void initializeEEPROM() {
    ...
    if (initializedFlag != 0xAA) {
        ...
        // Seuils stockés en uint16_t (2 octets)
        uint16_t drop_val = (uint16_t)(DROP_THRESHOLD * 10);
        uint16_t ret_val  = (uint16_t)(RETURN_THRESHOLD * 10);

        EEPROM.put(EEPROM_DROP_THRESHOLD_ADDR, drop_val);
        EEPROM.put(EEPROM_RETURN_THRESHOLD_ADDR, ret_val);

        initializedFlag = 0xAA;
        EEPROM.put(EEPROM_INITIALIZED_FLAG, initializedFlag);

        // Sauvegarde des changements (critique sur ESP32)
        EEPROM.commit();
...
void loop() {
    ...
    // Commandes reçues par le port série
...
void readAndFilterVoltage() {
    ...
    // Mode simulation : bypass la lecture ADC réelle
    if (simulation_mode) {
        simulateSineWave();
        return;
    }
    ...
    // --- MISE À L'ÉCHELLE ADS1115 ---
    // 1. Tension mesurée après diviseur : V_divided = (raw_value * ADS_GAIN_V) / ADS_RESOLUTION_MAX
    float V_divided = ((float)adc_raw_value * ADS_GAIN_V) / ADS_RESOLUTION_MAX;

    // 2. Tension plasma réelle : V_plasma = V_divided * PLASMA_VOLTAGE_DIVIDER_RATIO
    float raw = V_divided * PLASMA_VOLTAGE_DIVIDER_RATIO;

    // === FAST : sur-échantillonnage + low-pass (entrée du PID) ===
    oversample_sum += raw;
    oversample_count++;
    if (oversample_count >= OVERSAMPLE_TARGET) {
        ...
        // === SLOW : moyenne glissante sur 200 échantillons + low-pass (référence anti-dive) ===
        ...
        // Réinitialisation de la référence lente sur le front montant d'Enable :
        // l'arc est déjà stabilisé côté G-code, on capture donc la référence ici.
        ...
        slow_sum -= slow_samples[slow_idx]; // Retrait de la plus ancienne valeur de la somme
        slow_samples[slow_idx] = avg_raw;   // Ajout de la nouvelle valeur dans le tableau
        slow_sum += avg_raw;                // Ajout de la nouvelle valeur à la somme
        slow_idx = (slow_idx + 1) % N_SLOW; // Décalage circulaire de l'index

        float slow_raw_avg = slow_sum / N_SLOW; // Moyenne glissante
        // Low-pass appliqué sur la moyenne
        ...
        Input = fast_voltage;
        input_ready = true;  // Signale que Input est frais
        interrupts();
        // Reset du cycle d'oversampling
        oversample_sum = 0.0f;
        oversample_count = 0;
    }

    // Activation / désactivation de l'anti-dive
    if (warmed_up &&
...
void managePlasmaAndTHC() {
    ...
    // État du THC : dépend uniquement d'Enable et de la détection d'arc
    // (l'anti-dive et thc_off ne sont plus pris en compte ici, cf. la logique de commande ci-dessous)
    thc_active = enable_active && arc_voltage_ok;

    // --- LOGIQUE DE COMMANDE DU MOTEUR Z ---
    ...
    if (anti_dive_active) {
        ...
        // --- MODE 3 : PASSTHROUGH (la CNC prend le contrôle, pilotage DIRECT) ---
        if (pid_running) {
            myPID.SetMode(MANUAL);
            Output = 0.0;
            pid_running = false;
    ...
    // Sortie périodique pour le plotter série (throttlée à 30ms)
